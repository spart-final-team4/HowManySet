...
extension Int {
    ...
    func toMinutesLabel() -> String {
        let minutes = self / 60
        let format = String(localized: "%d분")
        return String(format: format, minutes)
    }
    
    /// Int 값을 0:00 or 0:00:00 형식의 Text(timeInterval:) 고정 형식으로 나타냄
    /// LiveActivity에서도 사용되어 Target 추가되어 있음
    func toWorkOutTimeLabelInLive() -> String {
        let hour = self / 3600
        let minute = (self % 3600) / 60
        let second = self % 60
    ...
    /// Int 값을 0:00 형식의 Text(timeInterval:) 고정 형식으로 나타냄
    /// LiveActivity에서도 사용되어 Target 추가되어 있음
    func toRestTimeLabelInLive() -> String {
